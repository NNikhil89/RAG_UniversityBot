...
const ChatInterface = ({ universityName, selectedLanguage }) => {
  const formattedUniversityName = universityName.replace(/-/g, ' ').toUpperCase();
  const [messages, setMessages] = useState([
    {
      origin: 'bot',
      message: `Hello! Welcome to ${formattedUniversityName} Bot, how can I assist you today?\n\n**Disclaimer: This chatbot service is not intended for private or confidential information.**`,
    },
  ]);
  ...
  const sendMessageToAPI = async (userMessage) => {
    ...
    try {
      var urlname = universityName.includes('deepseek') ? 
                    'http://127.0.0.1:5000/deepseek':
                      `http://127.0.0.1:5000/${universityName}`;
...
const formatText = (text) => {
  ...
  return (
    <div className="chat-container">
      ...
      <p className="disclaimer">This ChatBot can generate inaccurate answers. Check important info.</p>
    </div>
  );
};
