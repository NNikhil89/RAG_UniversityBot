...
model_choices = {
    ...
    "deepseek_chat_model" :"deepseek/deepseek-chat:free"
}

model_name = model_choices["deepseek_chat_model"]
...
@routes.route('/deepseek', methods=['POST'])
def deepseek_response():
    try:
        ...
        payload = {
            "model": "deepseek/deepseek-chat:free",  # Adjust as needed
        ...
        print('api key is', config.DEEPSEEK_API_KEY)
        print('url is', deepseek_url)
        response = requests.post(deepseek_url, json=payload, headers=headers)
        response_data = response.json()  
        print('res is ', response_data)
        #print('res is ', response_data["choices"][0]["message"]["content"])      
